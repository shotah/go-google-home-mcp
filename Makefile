# go-google-home-mcp Makefile — same shape as go-garmin
#
...
.PHONY: help fmt vet lint test test-race test-short coverage check \
	build cli install tidy deps clean install-hooks tools run release version
...
help: ## Show this help
	...
	@echo   cli                    Build the ghome CLI binary into ./bin/ghome
	@echo   install                Install ghome into GOPATH/bin
	@echo   run                    go run CLI  (make run ARGS="--help")
	@echo   run-mcp                Build then run MCP over stdio
	@echo.
...
cli: ## Build the ghome CLI binary into ./bin/ghome
	mkdir -p bin
	go build -ldflags "$(LDFLAGS)" -o bin/ghome ./cmd/ghome

install: ## Install ghome into $$GOPATH/bin (or $$GOBIN)
	go install -ldflags "$(LDFLAGS)" ./cmd/ghome

run: ## Build & run CLI — e.g. make run ARGS="devices"
	go run -ldflags "$(LDFLAGS)" ./cmd/ghome $(ARGS)

run-mcp: cli ## Run MCP server over stdio (requires prior ghome login)
	./bin/ghome mcp
...
clean: ## Remove built binaries and coverage artifacts
	go clean ./...
ifeq ($(OS),Windows_NT)
	-cmd /C "rmdir /S /Q bin 2>NUL & del /Q ghome.exe coverage.out coverage.txt 2>NUL"
else
	rm -rf bin
	rm -f ghome ghome.exe coverage.out coverage.txt
endif
